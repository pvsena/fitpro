...
import { Target, Dumbbell, TrendingUp, ChevronRight, Zap, Crown, Scale, Ruler, Calendar } from "lucide-react";
...
export default async function DashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="px-4 md:px-8 max-w-6xl mx-auto space-y-6 md:space-y-8">
        {/* Stats Row - Desktop: 4 columns, Mobile: 3 columns */}
        <div className="grid grid-cols-3 md:grid-cols-4 gap-3 md:gap-4">
          <div className="card-elevated p-4 text-center">
            <div className="text-2xl md:text-3xl font-bold text-primary">{planCount || 0}</div>
            <div className="text-xs md:text-sm text-muted-foreground">Planos</div>
          </div>
          <div className="card-elevated p-4 text-center">
            <div className="text-2xl md:text-3xl font-bold">{profile.weight}</div>
            <div className="text-xs md:text-sm text-muted-foreground">kg</div>
          </div>
          <div className="card-elevated p-4 text-center">
            <div className="text-2xl md:text-3xl font-bold">{profile.training_days_per_week}x</div>
            <div className="text-xs md:text-sm text-muted-foreground">semana</div>
          </div>
          <div className="hidden md:block card-elevated p-4 text-center">
            <div className="text-2xl md:text-3xl font-bold">{profile.height}</div>
            <div className="text-xs md:text-sm text-muted-foreground">cm</div>
          </div>
        </div>

        {/* Main Content Grid */}
        <div className="grid md:grid-cols-3 gap-6">
          {/* Left Column - Current Plan (spans 2 cols on desktop) */}
          <div className="md:col-span-2 space-y-6">
            {/* Current Goal Card */}
            <div className="card-elevated p-4 md:p-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 md:gap-4">
                  <div className="w-12 h-12 md:w-16 md:h-16 rounded-xl bg-primary/20 flex items-center justify-center text-2xl md:text-3xl">
                    {goalIcons[profile.goal]}
                  </div>
                  <div>
                    <div className="text-sm text-muted-foreground">Objetivo atual</div>
                    <div className="font-semibold md:text-lg">{goalLabels[profile.goal]}</div>
                  </div>
                </div>
                <Link href="/profile">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h2 className="font-semibold md:text-lg">Plano Atual</h2>
                {latestPlan && (
              ...
              {latestPlan ? (
                <Link href={`/plan/${latestPlan.id}`}>
                  <div className="card-elevated-hover p-4 md:p-6">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        <h3 className="font-semibold md:text-lg mb-1">{latestPlan.name}</h3>
                ...
                <div className="card-elevated p-6 md:p-8 text-center">
                  ...
                  <h3 className="font-semibold md:text-lg mb-2">Nenhum plano ativo</h3>
          ...
          {/* Right Column - Actions & Premium */}
          <div className="space-y-6">
            {/* Quick Actions */}
            <div>
              <h2 className="font-semibold md:text-lg mb-3">Ações Rápidas</h2>
            ...
            <div className="hidden md:block card-elevated p-4">
            ...
            <div className="card-elevated overflow-hidden">
              <div className="bg-gradient-to-r from-primary/20 to-primary/5 p-4">
                ...
                <Link href="/waitlist" className="block mt-4">
